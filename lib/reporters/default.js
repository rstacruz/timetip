...
Reporter.prototype.day = function(data, options) {
  ...
  // "Today"
  var rel;
  if (!today) rel = data.date.relative();
  else rel = data.date.format(this.log.formats.time);

  heading += c(" "+dash+" ", c.grey);
  heading += c(rel, c.blue);

  var total = c(chk+"  ", c.grey) + c(duration(data.summary.productive), c.blue);

  pf("");
  pf("  %-55s%15s", heading, total);
  pf("");
