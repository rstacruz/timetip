...
Reporter.prototype.summary = function(summary, data) {
  ...
  if (!data.range)
    ...
    heading = f('%s - %s',
      data.range[0].format('{mon} {d} {dow}'),
      data.range[1].format('{mon} {d} {dow}'));
  else if (data.range.getHours)
    heading = f('%s',
      data.range.format('{mon} {d} {dow}'));

  pf('  %s', T.accent(heading));
  ...
  if (_.size(projects) === 0) {
    ...
    _.each(projects, function(project, name) {
      ...
      // if (dur === '0h') dur = '<1h';
      var dur = duration(project.duration);
      pf ('    %8s  %s', T.mute(dur), name);
    });
  }
};
